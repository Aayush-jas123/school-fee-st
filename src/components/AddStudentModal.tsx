  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800/60 rounded-3xl max-w-2xl w-full p-6 shadow-2xl shadow-black/40 overflow-y-auto max-h-[90vh] text-zinc-100 relative">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-zinc-800/50 pb-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Enrol new student in JBT or B.Ed program</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            <label className="block text-zinc-300 font-medium mb-1.5">Select Program Course</label>
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className={`p-4 rounded-2xl border text-left flex items-center gap-3 transition-all ${
                  course === 'JBT'
                    ? 'bg-violet-600/20 border-violet-500/30 text-white shadow-lg shadow-violet-500/15 ring-1 ring-violet-500/40'
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500">2 Years Diploma • Annual Fee: ₹65,000</p>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500">2 Years Degree • Annual Fee: ₹78,000</p>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-zinc-300 font-medium mb-1">Father's / Guardian Name *</label>
              <input
                type="text"
                value={fatherName}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-zinc-300 font-medium mb-1">Contact Phone Number *</label>
              <input
                type="text"
                value={phone}
            ...
            <div>
              <label className="block text-zinc-300 font-medium mb-1">WhatsApp No.</label>
              <input
                type="text"
                value={whatsappNo}
                onChange={(e) => setWhatsappNo(e.target.value)}
                placeholder="e.g. 9876543210"
                className="w-full bg-zinc-950/60 border border-zinc-800/50 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-emerald-500"
              />
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div>
              <label className="block text-zinc-300 font-medium mb-1">Stream</label>
              <select
                value={stream}
                onChange={(e) => setStream(e.target.value)}
                className="w-full bg-zinc-950/60 border border-zinc-800/50 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-violet-500/50 cursor-pointer"
              >
            ...
            <div>
              <label className="block text-zinc-300 font-medium mb-1">Session</label>
              <select
                value={session}
            ...
            <div>
              <label className="block text-zinc-300 font-medium mb-1">Class Roll No.</label>
              <input
                type="text"
                value={rollNoInput}
                onChange={(e) => setRollNoInput(e.target.value)}
                placeholder="Auto or Manual"
                className="w-full bg-zinc-950/60 border border-zinc-800/50 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-violet-500/50 font-mono text-xs"
              />
            </div>
          </div>
          ...
          {/* Residential Address */}
          <div>
            <label className="block text-zinc-300 font-medium mb-1">Residential Address</label>
            <input
              type="text"
              value={address}
          ...
          <div className="bg-violet-950/20 border border-violet-800/40 p-4 rounded-2xl space-y-2">
            <label className="block text-violet-300 font-bold">Initial Token / Admission Payment (Optional ₹)</label>
            <p className="text-[11px] text-zinc-500">Enter amount if student is paying admission fee right now</p>
            <input
              type="number"
              min={0}
              max={defaultTotalFee}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-800/50">
            ...
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold shadow-lg shadow-violet-600/20 flex items-center gap-2 cursor-pointer"
            >
