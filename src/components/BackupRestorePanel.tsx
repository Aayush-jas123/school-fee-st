    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-neutral-900">Backup & Data Center</h2>
              <p className="text-xs text-neutral-500 mt-0.5">Download backups, restore data, and view session-wise records</p>
            </div>
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="p-3 rounded-xl bg-neutral-50 border border-neutral-100">
            <div className="flex items-center gap-2 text-neutral-500 text-[10px] font-semibold uppercase tracking-wider">
              <HardDrive className="w-3.5 h-3.5" /> Local Storage
            </div>
            <p className="text-sm font-bold text-neutral-900 mt-1">{storageUsed}</p>
          </div>
          <div className="p-3 rounded-xl bg-neutral-50 border border-neutral-100">
            <div className="flex items-center gap-2 text-neutral-500 text-[10px] font-semibold uppercase tracking-wider">
              <Users className="w-3.5 h-3.5" /> Total Students
            </div>
            <p className="text-sm font-bold text-neutral-900 mt-1">{students.length}</p>
          </div>
          <div className="p-3 rounded-xl bg-neutral-50 border border-neutral-100">
            <div className="flex items-center gap-2 text-neutral-500 text-[10px] font-semibold uppercase tracking-wider">
              <Calendar className="w-3.5 h-3.5" /> Sessions
            </div>
            <p className="text-sm font-bold text-neutral-900 mt-1">{sessionGroups.length}</p>
          </div>
          <div className="p-3 rounded-xl bg-neutral-50 border border-neutral-100">
            <div className="flex items-center gap-2 text-neutral-500 text-[10px] font-semibold uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-neutral-900">Download Full Backup</h3>
              <p className="text-[11px] text-neutral-500">Export all students, fee rules & logs as JSON</p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full px-4 py-3 rounded-xl bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-bold shadow-sm flex items-center justify-center gap-2 cursor-pointer transition-all duration-200 hover:-translate-y-0.5"
          >
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-neutral-900">Restore from Backup</h3>
              <p className="text-[11px] text-neutral-500">Load a previously exported backup file</p>
            </div>
          </div>
      ...
      >
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-sm font-bold text-neutral-900">Session-wise Data Browser</h3>
            <p className="text-[11px] text-neutral-500">View all student records organized by academic session</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {sessionStats.map(({ session, count, totalFees, totalPaid, totalPending, paidCount, unpaidCount, partlyPaidCount }) => {
            ...
            return (
              <div key={session} className="border border-neutral-200 rounded-2xl overflow-hidden">
                {/* Session Header */}
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="text-sm font-bold text-neutral-900">{session}</p>
                      <p className="text-[11px] text-neutral-500">{count} students</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-[10px] text-neutral-500">of {formatCurrencyINR(totalFees)}</p>
                    </div>
                    {isExpanded
                ...
                {isExpanded && (
                  <div className="border-t border-neutral-100 bg-neutral-50/50">
                    {/* Session Stats Bar */}
                    <div className="grid grid-cols-3 gap-px bg-neutral-200 border-b border-neutral-200">
                      <div className="bg-white p-3 text-center">
                        <p className="text-[10px] text-neutral-400 font-semibold uppercase">Expected</p>
                        <p className="text-sm font-bold text-neutral-900">{formatCurrencyINR(totalFees)}</p>
                      </div>
                      <div className="bg-white p-3 text-center">
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      <table className="w-full text-xs">
                        <thead className="sticky top-0 bg-neutral-100 z-10">
                          <tr>
                            <th className="text-left px-4 py-2 text-neutral-500 font-semibold">Reg. No</th>
                            <th className="text-left px-4 py-2 text-neutral-500 font-semibold">Name</th>
                            <th className="text-left px-4 py-2 text-neutral-500 font-semibold hidden md:table-cell">Course</th>
                            <th className="text-right px-4 py-2 text-neutral-500 font-semibold">Total</th>
                            <th className="text-right px-4 py-2 text-neutral-500 font-semibold">Paid</th>
                            <th className="text-right px-4 py-2 text-neutral-500 font-semibold">Pending</th>
                            <th className="text-center px-4 py-2 text-neutral-500 font-semibold">Status</th>
                          </tr>
                        </thead>
                        <tbody>
                          {sessionStudents.map((student) => (
                            <tr key={student.id} className="border-t border-neutral-100 hover:bg-white transition-colors">
                              <td className="px-4 py-2.5 font-mono text-neutral-700">{student.registrationNo}</td>
                              <td className="px-4 py-2.5 font-medium text-neutral-900">{student.name}</td>
                              <td className="px-4 py-2.5 text-neutral-600 hidden md:table-cell">
