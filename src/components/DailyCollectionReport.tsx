    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-2xl bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 flex items-center justify-center font-bold shadow-inner">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-indigo-600/30 cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-800/80 text-xs">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800 font-medium">
              <button
                onClick={() => { setDateFilterMode('today'); setSelectedDate(todayStr); }}
                className={`px-3 py-1.5 rounded-lg transition-all font-bold ${
                  dateFilterMode === 'today' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                }`}
              >
                Today ({todayStr})
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-all font-bold ${
                  dateFilterMode === 'all' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                }`}
              >
                All Records
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-all font-bold ${
                  dateFilterMode === 'custom' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                }`}
              >
            ...
            {dateFilterMode === 'custom' && (
              <input
                ...
                className="bg-slate-950 border border-indigo-500/60 rounded-xl px-3 py-1.5 text-white font-mono font-bold focus:outline-none"
              />
            )}

            {/* Mode Filter */}
            <div className="flex items-center gap-1 bg-slate-950 p-1.5 rounded-xl border border-slate-800 font-medium">
              <span className="text-slate-400 px-1 font-semibold">Mode:</span>
              {(['ALL', 'Cash', 'UPI', 'NEFT', 'Cheque', 'Demand Draft'] as const).map((m) => (
                <button
                  key={m}
                  onClick={() => setSelectedModeFilter(m)}
                  className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all ${
                    selectedModeFilter === m ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {m}
                </button>
          ...
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-white font-medium text-xs focus:outline-none focus:border-indigo-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Total Collection</span>
            <div className="w-8 h-8 rounded-xl bg-indigo-500/20 text-indigo-400 flex items-center justify-center font-bold">
              <DollarSign className="w-4 h-4" />
            </div>
          </div>
          <span className="text-2xl font-extrabold text-white">{formatCurrencyINR(totalCollected)}</span>
          <p className="text-[11px] text-slate-400 mt-1">{filteredTransactions.length} payment records logged</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Cash Collections</span>
            <div className="w-8 h-8 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold">
              <CheckCircle2 className="w-4 h-4" />
            </div>
          </div>
          <span className="text-2xl font-extrabold text-emerald-400">{formatCurrencyINR(cashCollected)}</span>
          <p className="text-[11px] text-slate-400 mt-1">Physical cashier desk cash</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">UPI / Online QR</span>
            <div className="w-8 h-8 rounded-xl bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold">
              <QrCode className="w-4 h-4" />
            </div>
          </div>
          <span className="text-2xl font-extrabold text-blue-400">{formatCurrencyINR(upiCollected)}</span>
          <p className="text-[11px] text-slate-400 mt-1">GPay, PhonePe, Paytm QR</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Bank / Cheque / DD</span>
            <div className="w-8 h-8 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold">
              <Building className="w-4 h-4" />
            </div>
          </div>
          <span className="text-2xl font-extrabold text-amber-400">{formatCurrencyINR(neftCollected)}</span>
          <p className="text-[11px] text-slate-400 mt-1">NEFT, Cheques & Demand Drafts</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-4 md:p-5 border-b border-slate-800 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-mono">Accountant: {staffName}</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left text-slate-300">
                ...
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-500">
                    <div className="max-w-xs mx-auto space-y-1">
                      ...
                      <p className="font-bold text-slate-300">No payment transactions logged for {selectedDate}</p>
                      <p className="text-[11px] text-slate-500">Select another date or switch to "All Records" view.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredTransactions.map(({ student, payment }) => (
                  <tr key={`${student.id}-${payment.id}`} className="hover:bg-slate-800/50 transition-colors">
                    <td className="py-3 px-4 whitespace-nowrap">
                      <span className="font-mono font-bold text-indigo-300 block">{payment.id}</span>
                      <span className="text-[10px] text-slate-400 font-mono">{payment.date}</span>
                    </td>
                    <td className="py-3 px-4 whitespace-nowrap">
                      <span className="font-bold text-white block">{student.name}</span>
                      <span className="text-[10px] text-slate-400 font-mono">{student.registrationNo}</span>
                    </td>
                    <td className="py-3 px-4 whitespace-nowrap">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-500/20 text-blue-300 border border-blue-500/30 mr-1">
                        {student.course}
                      </span>
                      <span className="text-slate-400 text-[11px]">{student.stream || 'Arts'}</span>
                    </td>
                    <td className="py-3 px-4 whitespace-nowrap">
                    ...
                    <td className="py-3 px-4 text-center whitespace-nowrap">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-[11px] flex items-center gap-1 mx-auto cursor-pointer"
                      >
            ...
            {filteredTransactions.length > 0 && (
              <tfoot className="bg-slate-950 text-white font-extrabold border-t border-slate-700">
                <tr>
                  <td colSpan={6} className="py-3 px-4 text-right text-xs uppercase tracking-wider text-slate-300">
