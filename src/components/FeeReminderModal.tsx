...
export const FeeReminderModal: React.FC<FeeReminderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-50/70 backdrop-blur-md animate-fadeIn">
      <div className="bg-white border border-stone-200 rounded-3xl max-w-xl w-full p-6 shadow-2xl overflow-hidden text-stone-900 relative">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-stone-200 pb-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-stone-900 tracking-tight">Fee Reminder & Demand Notice</h2>
              <p className="text-xs text-stone-500">Send instant reminder or print formal letter</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Student Alert Box */}
        <div className="bg-stone-50 border border-stone-200 rounded-2xl p-4 mb-6 space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-stone-900 text-sm">{student.name} ({student.course})</span>
            <span className="px-2 py-0.5 rounded-full bg-stone-100 text-stone-700 font-bold uppercase text-[10px]">
              {student.feeStatus}
            </span>
          </div>
          <div className="flex items-center justify-between text-xs text-stone-500">
            <span>Outstanding Dues:</span>
            <strong className="text-stone-700 font-bold text-sm">{formatCurrencyINR(student.remainingFees)}</strong>
          </div>
          <div className="flex items-center justify-between text-xs text-stone-500">
        ...
        <div className="space-y-4 text-xs">
          <div>
            <label className="block text-stone-700 font-medium mb-1.5">Notification Method</label>
            <div className="grid grid-cols-4 gap-2">
              {[
                { name: 'WhatsApp', icon: MessageSquare, color: 'text-stone-700' },
                { name: 'Email', icon: Mail, color: 'text-stone-700' },
                { name: 'SMS', icon: Phone, color: 'text-stone-700' },
                { name: 'Printed Notice', icon: Printer, color: 'text-stone-700' }
              ].map((c) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-2xl border flex flex-col items-center gap-1.5 transition-all text-center ${
                      channel === c.name
                        ? 'bg-rose-800 border-stone-200 text-stone-50 shadow-lg shadow-rose-800/15'
                        : 'bg-stone-50/60 border-stone-200 text-stone-500 hover:border-stone-200'
                    }`}
                  >
                    <IconComponent className={`w-5 h-5 ${channel === c.name ? 'text-stone-900' : c.color}`} />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-stone-700 font-medium mb-1">Late Penalty Fine (₹)</label>
              <input
                type="number"
                min={0}
                value={customLateFee}
                onChange={(e) => setCustomLateFee(Number(e.target.value))}
                className="w-full bg-stone-50/60 border border-stone-200 rounded-xl px-3 py-2 text-stone-900 font-bold focus:outline-none focus:border-rose-800/50"
              />
            </div>
            <div>
              <label className="block text-stone-700 font-medium mb-1">Grace Period (Days)</label>
              <input
                type="number"
                min={1}
                value={graceDays}
          ...
          {/* Message Preview Box */}
          <div>
            <label className="block text-stone-700 font-medium mb-1">Generated Message Template</label>
            <div className="bg-stone-50/60 p-3 rounded-xl border border-stone-200 text-stone-700 text-[11px] leading-relaxed font-sans">
              {getNoticeMessage()}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-stone-200">
            <button
              ...
              className="px-4 py-2 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold flex items-center gap-1.5 cursor-pointer text-xs"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-rose-800 hover:bg-rose-700 text-stone-50 font-bold shadow-lg shadow-rose-800/10 flex items-center gap-2 cursor-pointer text-xs"
            >
          ...
          <div className="fixed inset-0 z-60 bg-black/90 flex items-center justify-center p-4">
            <div className="bg-white text-slate-900 rounded-3xl max-w-2xl w-full p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto relative print:p-0 print:shadow-none">
              <button
                ...
                className="absolute right-6 top-6 p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-stone-600 print:hidden"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-2">
                ...
                <table className="w-full text-xs border-collapse border border-slate-300">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td className="border border-slate-300 p-2 text-stone-700">Less: Amount Received Till Date</td>
                      <td className="border border-slate-300 p-2 text-right text-stone-700">({formatCurrencyINR(student.paidTillNow)})</td>
                    </tr>
                    <tr>
                      <td className="border border-slate-300 p-2 font-bold text-stone-900">Net Outstanding Balance Fee</td>
                      <td className="border border-slate-300 p-2 text-right font-bold text-stone-900">{formatCurrencyINR(student.remainingFees)}</td>
                    </tr>
                    <tr>
              ...
              <div className="text-[11px] text-stone-600 space-y-2">
              ...
              <div className="pt-8 flex justify-between items-end text-xs">
                <div>
                  ...
                  <p className="text-stone-600 text-[10px]">Official Institutional Seal</p>
                </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 print:hidden">
                <button
                  ...
                  className="px-6 py-2.5 bg-rose-800 hover:bg-rose-700 text-stone-50 rounded-xl font-bold text-xs flex items-center gap-2"
                >
