...
export const StudentReceiptModal: React.FC<StudentReceiptModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-3 md:p-6 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-4xl shadow-2xl text-slate-100 overflow-hidden my-auto">
        {/* Header */}
        <div className="p-5 md:p-6 bg-gradient-to-r from-slate-950 to-indigo-950/40 border-b border-slate-800 flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                ...
                <span className="text-indigo-300 font-bold">{student.name}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 bg-slate-950/60 border-b border-slate-800 p-4 gap-3 text-xs">
          ...
          <div>
            <span className="text-slate-400 block text-[10px] uppercase font-semibold">Registration No</span>
            <span className="text-sm font-bold font-mono text-indigo-300">{student.registrationNo}</span>
          </div>
          <div>
            <span className="text-slate-400 block text-[10px] uppercase font-semibold">Course / Semester</span>
            <span className="text-sm font-bold text-white flex items-center gap-1.5">
          ...
          <div>
            <span className="text-slate-400 block text-[10px] uppercase font-semibold">Total Paid / Remaining</span>
        ...
        <div className="flex border-b border-slate-800 bg-slate-950/30 px-6 gap-6 text-xs font-semibold text-slate-400">
          <button
            ...
            className={`py-3 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSection === 'payments' ? 'border-emerald-500 text-emerald-400 font-bold' : 'border-transparent hover:text-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`py-3 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSection === 'semester' ? 'border-indigo-500 text-indigo-400 font-bold' : 'border-transparent hover:text-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`py-3 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSection === 'statement' ? 'border-amber-500 text-amber-400 font-bold' : 'border-transparent hover:text-slate-200'
            }`}
          >
            ...
            <div className="space-y-4">
              {student.paymentHistory.length === 0 ? (
                <div className="text-center py-10 text-slate-500 space-y-3">
                  ...
                  <p className="text-xs text-slate-500">Receipts will be available once payments are recorded for this student.</p>
                </div>
              ) : (
                <>
                  ...
                  {selectedPaymentId && selectedPayment && (
                    <div className="sticky bottom-0 bg-slate-900/95 backdrop-blur-sm border-t border-slate-800 -mx-6 px-6 py-4 mt-4">
                      <div className="flex items-center justify-between">
                        <div className="text-xs text-slate-400">
                          Selected: <span className="font-bold text-emerald-400">{formatINR(selectedPayment.amount)}</span>
                          {' '}for <span className="font-bold text-indigo-300">{selectedPayment.targetSemester || 'Fee'}</span>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-slate-400 font-medium">({slot.year})</span>
                        </div>
                        <span
                      ...
                      {semPayments.length > 0 && (
                        <button
                          ...
                          className="w-full px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md shadow-indigo-600/20 transition-colors"
                        >
              ...
              <div className="bg-slate-950/80 p-5 rounded-2xl border border-slate-800 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <span className="text-slate-400 block text-[10px] uppercase font-semibold">Total Program Fee</span>
                  ...
                  <div className="bg-emerald-950/30 p-3 rounded-xl border border-emerald-800/30">
                    <span className="text-slate-400 block text-[10px] uppercase font-semibold">Total Collected</span>
                  ...
                  <div className="bg-indigo-950/30 p-3 rounded-xl border border-indigo-800/30">
                    <span className="text-slate-400 block text-[10px] uppercase font-semibold">Total Transactions</span>
                    <span className="text-base font-extrabold text-indigo-400 block mt-1">{student.paymentHistory.length}</span>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-800">
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y divide-slate-800 text-slate-300">
                      {semesterSlots.map((slot) => (
                        <tr key={slot.semester} className="hover:bg-slate-800/40">
                    ...
                    <tfoot className="bg-slate-900 font-bold border-t border-slate-700 text-white">
                      <tr>
                  ...
                  <table className="w-full text-xs text-left">
                    ...
                    <tfoot className="bg-slate-900 font-bold border-t border-slate-700 text-white">
                      <tr>
                        ...
                        <td className="p-2.5 text-right text-sm text-indigo-400">{formatINR(student.totalFees)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
        ...
        {/* Footer */}
        <div className="p-4 md:p-5 bg-slate-950 border-t border-slate-800 flex items-center justify-between text-xs">
