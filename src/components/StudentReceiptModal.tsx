...
export const StudentReceiptModal: React.FC<StudentReceiptModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-3 md:p-6 overflow-y-auto">
      <div className="bg-white border border-stone-200 rounded-3xl w-full max-w-4xl shadow-2xl text-stone-900 overflow-hidden my-auto">
        {/* Header */}
        <div className="p-5 md:p-6 bg-stone-100 border-b border-stone-200 flex items-start justify-between">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 bg-stone-50/50 border-b border-stone-200 p-4 gap-3 text-xs">
          <div>
            <span className="text-stone-500 block text-[10px] uppercase font-semibold">Student Name</span>
          ...
          <div>
            <span className="text-stone-500 block text-[10px] uppercase font-semibold">Registration No</span>
            <span className="text-sm font-bold font-mono text-stone-700">{student.registrationNo}</span>
          </div>
          <div>
            <span className="text-stone-500 block text-[10px] uppercase font-semibold">Course / Semester</span>
            <span className="text-sm font-bold text-stone-900 flex items-center gap-1.5">
          ...
          <div>
            <span className="text-stone-500 block text-[10px] uppercase font-semibold">Total Paid / Remaining</span>
        ...
        <div className="flex border-b border-stone-200 bg-stone-50/60/30 px-6 gap-6 text-xs font-semibold text-stone-500">
          <button
            ...
            className={`py-3 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSection === 'payments' ? 'border-stone-900 text-stone-700 font-bold' : 'border-transparent hover:text-stone-800'
            }`}
          >
          ...
          <button
            ...
            className={`py-3 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSection === 'semester' ? 'border-stone-200 text-stone-700 font-bold' : 'border-transparent hover:text-stone-800'
            }`}
          >
          ...
          <button
            ...
            className={`py-3 border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
              activeSection === 'statement' ? 'border-stone-500 text-stone-700 font-bold' : 'border-transparent hover:text-stone-800'
            }`}
          >
        ...
        <div className="p-6 max-h-[55vh] overflow-y-auto space-y-4">
          ...
          {activeSection === 'payments' && (
            <div className="space-y-4">
              {student.paymentHistory.length === 0 ? (
                <div className="text-center py-10 text-stone-600 space-y-3">
                  <AlertCircle className="w-10 h-10 text-stone-600 mx-auto" />
                  <p className="text-sm font-semibold text-stone-500">No payment records found</p>
                  <p className="text-xs text-stone-600">Receipts will be available once payments are recorded for this student.</p>
                </div>
              ) : (
                <>
                      ...
                      return (
                        <div
                          ...
                          className={`p-4 rounded-xl border cursor-pointer transition-all ${
                            isSelected
                              ? 'bg-stone-50 border-stone-400 ring-1 ring-stone-300'
                              : 'bg-stone-50/50 border-stone-200 hover:border-stone-200'
                          }`}
                        >
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                                isSelected ? 'bg-stone-100 text-stone-700' : 'bg-stone-100 text-stone-500'
                              }`}>
                                {isSelected ? <CheckCircle2 className="w-4 h-4" /> : <Hash className="w-4 h-4" />}
                              </div>
                              <div className="min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="font-mono font-bold text-stone-700 text-xs">{rec.id}</span>
                                  {rec.targetSemester && (
                                    <span className="px-1.5 py-0.5 rounded bg-stone-100 text-stone-700 font-bold border border-stone-200 text-[10px]">
                                      {rec.targetSemester}
                                    </span>
                                  )}
                                  {rec.installmentNo && (
                                  ...
                                  <span className="px-1.5 py-0.5 rounded bg-stone-100 text-stone-700 font-semibold text-[10px]">
                                    {rec.mode}
                                  </span>
                                </div>
                            ...
                            <div className="text-right shrink-0">
                              <span className="text-sm font-extrabold text-stone-700 block">
                                {formatINR(rec.amount)}
                              </span>
                              {rec.discountApplied && rec.discountApplied > 0 && (
                                <span className="text-[10px] text-stone-700 font-semibold">
                  ...
                  {selectedPaymentId && selectedPayment && (
                    <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-stone-200 -mx-6 px-6 py-4 mt-4">
                      <div className="flex items-center justify-between">
                        <div className="text-xs text-stone-500">
                          Selected: <span className="font-bold text-stone-700">{formatINR(selectedPayment.amount)}</span>
                          {' '}for <span className="font-bold text-stone-700">{selectedPayment.targetSemester || 'Fee'}</span>
                          {' '}on <span className="font-mono">{selectedPayment.date}</span>
                        </div>
                        <button
                          onClick={handleGenerateSelected}
                          className="px-5 py-2.5 rounded-xl bg-rose-800 hover:bg-rose-700 text-stone-50 font-bold text-xs flex items-center gap-2 cursor-pointer shadow-lg shadow-rose-800/15 transition-colors"
                        >
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-stone-500 font-medium">({slot.year})</span>
                        </div>
                        <span
                      ...
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          <span className="text-stone-500">Semester Fee:</span>
                          <span className="font-bold text-stone-900">{slot.totalFee > 0 ? formatINR(slot.totalFee) : 'TBD'}</span>
                        </div>
                        <div className="flex justify-between">
                      ...
                      <div className="w-full bg-white rounded-full h-1.5 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${
                            isPaid ? 'bg-rose-800/70' : isPartly ? 'bg-amber-700/60' : 'bg-stone-300'
                          }`}
                          style={{
                      ...
                      {semPayments.length > 0 && (
                        <button
                          ...
                          className="w-full px-3 py-2 rounded-xl bg-rose-800 hover:bg-rose-700 text-stone-50 font-semibold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md shadow-rose-800/15 transition-colors"
                        >
          ...
          {activeSection === 'statement' && (
            <div className="space-y-5">
              ...
              <div className="bg-stone-50/70 p-5 rounded-2xl border border-stone-200 space-y-4">
                <div className="flex items-center gap-3 border-b border-stone-200 pb-3">
                  <FileText className="w-5 h-5 text-stone-700" />
                  <h4 className="font-bold text-stone-900 text-sm">Complete Fee Statement Preview</h4>
                </div>

                {/* Summary Grid */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                  <div className="bg-white p-3 rounded-xl border border-stone-200">
                    <span className="text-stone-500 block text-[10px] uppercase font-semibold">Total Program Fee</span>
                    <span className="text-base font-extrabold text-stone-900 block mt-1">{formatINR(student.totalFees)}</span>
                  </div>
                  <div className="bg-stone-50 p-3 rounded-xl border border-stone-200">
                    <span className="text-stone-500 block text-[10px] uppercase font-semibold">Total Collected</span>
                    <span className="text-base font-extrabold text-stone-700 block mt-1">{formatINR(student.paidTillNow)}</span>
                  </div>
                  <div className="bg-stone-50 p-3 rounded-xl border border-stone-200">
                    <span className="text-stone-500 block text-[10px] uppercase font-semibold">Outstanding</span>
                    <span className="text-base font-extrabold text-stone-700 block mt-1">{formatINR(student.remainingFees)}</span>
                  </div>
                  <div className="bg-stone-50 p-3 rounded-xl border border-stone-200">
                    <span className="text-stone-500 block text-[10px] uppercase font-semibold">Total Transactions</span>
                    <span className="text-base font-extrabold text-stone-700 block mt-1">{student.paymentHistory.length}</span>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto rounded-xl border border-stone-200">
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y divide-stone-200 text-stone-700">
                      {semesterSlots.map((slot) => (
                        <tr key={slot.semester} className="hover:bg-stone-100/40">
                          <td className="p-2.5 font-bold text-stone-900">{slot.semester}</td>
                          ...
                          <td className="p-2.5 text-right font-mono text-stone-700">{formatINR(slot.paidAmount)}</td>
                          <td className="p-2.5 text-right font-mono font-bold text-stone-700">
                    ...
                    <tfoot className="bg-white font-bold border-t border-stone-200 text-stone-900">
                      <tr>
                        ...
                        <td className="p-2.5 text-right font-mono text-stone-700">{formatINR(student.paidTillNow)}</td>
                        <td className="p-2.5 text-right font-mono text-stone-700">{formatINR(student.remainingFees)}</td>
                ...
                <div className="overflow-x-auto rounded-xl border border-stone-200">
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y divide-stone-200 text-stone-700">
                      <tr><td className="p-2.5">Tuition & Academic Training</td><td className="p-2.5 text-right font-bold text-stone-900">{formatINR(student.feeBreakdown.tuitionFee)}</td></tr>
                      <tr><td className="p-2.5">Admission & Registration</td><td className="p-2.5 text-right font-bold text-stone-900">{formatINR(student.feeBreakdown.admissionFee)}</td></tr>
                      <tr><td className="p-2.5">University Examination</td><td className="p-2.5 text-right font-bold text-stone-900">{formatINR(student.feeBreakdown.examFee)}</td></tr>
                      <tr><td className="p-2.5">Library & Digital Resources</td><td className="p-2.5 text-right font-bold text-stone-900">{formatINR(student.feeBreakdown.libraryFee)}</td></tr>
                      <tr><td className="p-2.5">Campus Development Fund</td><td className="p-2.5 text-right font-bold text-stone-900">{formatINR(student.feeBreakdown.developmentFee)}</td></tr>
                      <tr><td className="p-2.5">Practical Lab & Skill Training</td><td className="p-2.5 text-right font-bold text-stone-900">{formatINR(student.feeBreakdown.labFee)}</td></tr>
                    </tbody>
                    <tfoot className="bg-white font-bold border-t border-stone-200 text-stone-900">
                      <tr>
                        <td className="p-2.5">Total 2-Year Fee</td>
                        <td className="p-2.5 text-right text-sm text-stone-700">{formatINR(student.totalFees)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
              ...
              <button
                ...
                className="w-full px-5 py-3 rounded-xl bg-rose-800 hover:bg-rose-700 text-stone-50 font-bold text-sm flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-rose-800/10 transition-all"
              >
        ...
        {/* Footer */}
        <div className="p-4 md:p-5 bg-stone-50/60 border-t border-stone-200 flex items-center justify-between text-xs">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold transition-colors cursor-pointer"
          >
            Close
          </button>
        </div>
